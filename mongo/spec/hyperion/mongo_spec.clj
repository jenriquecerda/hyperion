  ...
  (context "Client connection"
    ...
    (it "can open client with one address"
      (with-open [mongo (open-mongo :host "127.0.0.1" :port 27017)]
        (should= com.mongodb.MongoClient (class mongo))
    ...
    (it "can open client with multiple addresses"
      (pending "Fails after mongodb upgrade. Of course, foo.com is not reachable.")
    ...
    (it "opens a client with credentials"
      (pending "Need to create a test db with valid users, or add instructions to do it manually")

      (with-open [mongo (open-mongo :host "127.0.0.1" :port 27017 :username "" :password "" :database "")]
        (should= com.mongodb.MongoClient (class mongo))
        (should= [["127.0.0.1" 27017]] (map address->seq (.getAllAddress mongo)))))
